...
1. **Unverified push-hook coverage.** It is not empirically confirmed that `check-code-alignment` fires when a Task-dispatched sub-agent performs Write/Edit on a source file. If it does, we document the guarantee. If it doesn't, the gap becomes the trigger for a separate snapshot-injection plan.
...
- **Option B snapshot injection** at Task dispatch — revisit only if B1 shows hooks don't fire.
...
- `test/structure/agents.bats` — assert the literal phrase `most common tags` (or similarly unique anchor) is present in both agent files.

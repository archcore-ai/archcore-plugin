...
@test "session-start uses shim variable" {
  grep -q '"\$SHIM"' "$PLUGIN_ROOT/bin/session-start"
}

@test "validate-archcore uses shim variable" {
  grep -q '"\$SHIM"' "$PLUGIN_ROOT/bin/validate-archcore"
}
